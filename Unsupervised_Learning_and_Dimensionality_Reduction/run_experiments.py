...
def load_dataset(dataset='WDBC', split_percentage=0.2):
    """Load CreditApproval or WDBC.

       Args:
           dataset (string): dataset, WDBC or CreditApproval.
    ...
    datasets = ['WDBC', 'CreditApproval']  # datasets names
    ...
    if dataset == datasets[0]:
        ...
        # Load CreditApproval Disease Data
        data = pd.read_csv('./data/credit_a3.csv')

        x = data.loc[:, data.columns != 'Approved']
        y = data.loc[:, data.columns == 'Approved']

        # Generate a statified smaller subset of CreditApproval
        x, _, y, _ = train_test_split(x, y, test_size=0.20, shuffle=True, random_state=42, stratify=y)
        y = y.values.ravel()
...
if __name__ == '__main__':
    ...
    # Run experiment 2 on CreditApproval
    print('\n--------------------------')
    dataset = 'CreditApproval'
    perform_model_complexity = False
    x_train, x_test, y_train, y_test = load_dataset(dataset)
    ...
    x_pca, x_ica, x_kpca, x_rp = dimensionality_reduction(x_train, x_test, y_train,
                                                          dataset=dataset,
                                                          pca_n_components=4, pca_kmeans_n_clusters=2,
                                                          pca_em_n_clusters=2, pca_em_covariance='diag',
                                                          ica_n_components=4, ica_kmeans_n_clusters=2,
                                                          ica_em_n_clusters=5, ica_em_covariance='diag',
                                                          kpca_n_components=4, kpca_kernel='cosine',
                                                          kpca_kmeans_n_clusters=2,
                                                          kpca_em_n_clusters=2, kpca_em_covariance='diag',
                                                          rp_n_components=5, rp_kmeans_n_clusters=2,
    ...
    neural_network(x_train, x_test, y_train, y_test,
                   x_pca, x_ica, x_kpca, x_rp,
                   kmeans_clusters, gmm_clusters,
                   layer1_nodes=100, layer2_nodes=80, learning_rate=0.01)
